...
class StationaryDependentSEM(SEMBase):
    @staticmethod
    def static():
...
class LinearMultipleChildrenSEM(SEMBase):
    """
...
class StationaryDependentMultipleChildrenSEM(SEMBase):
    """
    Test DAG for nodes within a slice that have more than one child _within_ the slice.
...
class StationaryIndependentSEM(SEMBase):
    @staticmethod
    def static():
        X = lambda noise, t, sample: noise
...
class NonStationaryDependentSEM(SEMBase):
    """
...
class NonStationaryIndependentSEM(SEMBase):
    """
    This SEM currently supports one change point.
...
if __name__ == "__main__":
    func = StationaryIndependentSEM().static()
    dx = np.array([ 1.1110234 , -0.99310243,  0.92221826,  0.84516495,  1.146313  ])
    dz = np.array([ 1.5121326,  2.4375143, -2.9636757,  1.1841679, -2.2486467])
    
    sample = {"X": dx, "Z": dz}
    noise = 0
    t = 0
    print(func["Y"](noise, t, sample))