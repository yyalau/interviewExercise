...
class PriorTrans(PriorBase):
    ...
    def fork_ops(self,  pa_node, pa_value, ch_node, ch_value, i, funcs):
        ...
        if pa_node.t != ch_node.t:
            ...
            funcs[k][ch_node.t, 0] = GPRegression().fit(x=pa_value, y=ch_value)
    ...
    def normal_ops(self, pa_node, pa_value, ch_node, ch_value, funcs):
        ...
        funcs[k][pa_node.t, 0] = GPRegression().fit(x=pa_value[...,None], y=ch_value)
    ...
    def collider_ops(self, pa_nodes, pa_values, ch_node, ch_value, funcs):
        ...
        if pa_nodes[0].t != ch_node.t:
            ...
            funcs[k][pa_nodes[0].t, 0] = GPRegression().fit(x=pa_values[...,None], y=ch_value)
    ...
    def get_M(self):
        ...
        for t in range(self.nT -1):
            A[t*self.nVar : (t+1)*self.nVar, (t+1)*self.nVar : (t+2)*self.nVar] = M[t*self.nVar : (t+1)*self.nVar, (t+1)*self.nVar : (t+2)*self.nVar]
        return A
